...
abstract class ArcTouchAdapter(private val dataBindingComponent: DataBindingComponent? = null) :
        RecyclerView.Adapter<ArcTouchAdapter.ViewHolder>() {

    protected val disposables = CompositeDisposable()

    open var items: List<Any>? = null
        protected set
    ...
    @MainThread
    open fun replace(update: List<Any>?) {
        dataVersion++
        when {
            items == null -> {
    ...
    inner class Task(
        ...
        override fun doInBackground(vararg voids: Void?): DiffUtil.DiffResult =
                DiffUtil.calculateDiff(object : DiffUtil.Callback() {
                    ...
                    override fun areItemsTheSame(oldItemPosition: Int, newItemPosition: Int) =
                            this@ArcTouchAdapter.areItemsTheSame(
                                    oldItems[oldItemPosition],
                                    newItems[newItemPosition]
                            )

                    override fun areContentsTheSame(oldItemPosition: Int, newItemPosition: Int) =
                            this@ArcTouchAdapter.areContentsTheSame(
                                    oldItems[oldItemPosition],
                                    newItems[newItemPosition]
                            )
                })
        ...
        override fun onPostExecute(result: DiffUtil.DiffResult) {
            if (startVersion == dataVersion) {
                items = newItems
                result.dispatchUpdatesTo(this@ArcTouchAdapter)
            }
        }
    }
}